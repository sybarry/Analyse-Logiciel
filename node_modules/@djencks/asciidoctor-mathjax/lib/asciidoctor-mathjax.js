...
function processStemBlock (processor, doc, stem) {
  ...
  const typeset = stem.style === 'asciimath'
    ? stem.lines.map((line) => processor.createMathSrc(line, stem.style, doc, true)).join(Break)
    : processor.createMathSrc(stem.lines.join('\n'), stem.style, doc, true)
  const parent = stem.parent
  const title = typeof stem.title === 'string' ? `<div class="title">${stem.title}</div>\n    ` : ''
  const classes = typeof stem.attributes.$$smap.role === 'string' ? ` ${stem.attributes.$$smap.role}` : ''
  const stemBlock = processor.$create_pass_block(
    parent,
    `  <div class="stemblock${classes}">
    ${title}<div class="content halign-center valign-middle" style="padding-top: 1rem">
      ${typeset}
    </div>
  </div>`,
...
function processInlineStem (processor, node, text) {
  ...
  const mathText = !(text && text.replace)
    ...
    : text.replace(StemInlineMacroRx, (match, g0, g1, g2, index, string) => {
      ...
      return `pass:[<span class="steminline"> ${processor.createMathSrc(eqData, stemStyle, node.getDocument(), false)} </span>]`
    })
  ...
  return (math, style, doc, display) => {
    ...
    if (!input) {
      if (style === 'asciimath') {
        ...
        const packagesString = doc.getAttribute('mathjax-tex-packages')
        const packages = ['base']
        if (packagesString) {
          packagesString.split(' ').forEach((candidate) => {
            if (candidate) {
              const requireName = `mathjax-full/js/input/tex/${candidate}/${candidate.charAt(0).toUpperCase()}${candidate.slice(1)}Configuration.js`
              try {
                require(requireName)
                packages.push(candidate)
              } catch (e) {
                console.log(`unknown mathjax tex package ${candidate}`, e)
              }
            }
          })
        }
        const tags = doc.getAttribute('mathjax-tex-tags') || 'none'
        const latexmath = new Tex({ packages, tags })
